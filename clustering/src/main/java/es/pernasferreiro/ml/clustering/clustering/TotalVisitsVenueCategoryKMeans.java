...
public class TotalVisitsVenueCategoryKMeans {
		...
		if (args.length > 0) {
			...
			if (
				...
				JavaPairRDD<String, Vector> data = dataset.toJavaRDD().mapToPair(
						(in) -> {
							String par = in.getString(in.fieldIndex("VenueCategoryID")) + " " + in.getString(in.fieldIndex("UserID")) + " " + String.valueOf(in.getLong(in.fieldIndex("VisitasTotales")));
