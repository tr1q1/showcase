...
public class TotalVisitsLastVisitAggregate {
	...
	public static void main(String[] args) {
		if (args.length > 0) {
			...
			if (
				...
				java.sql.Timestamp startTime = new java.sql.Timestamp(Calendar.getInstance().getTimeInMillis());
				StringBuilder sql = new StringBuilder(0);
				sql.append("SELECT VenueID, UserID, COUNT(*) AS VisitasTotales, (DATEDIFF(TO_DATE('" + startTime + "'), MAX(UTCTime))) AS DiasDesdeUltimaVisita ");
