...
public class TotalVisitsLastVisitBisectingKMeans {
	...
	public static void main(String[] args) {
		if (args.length > 0) {
			...
			if (
				...
				conf.set("spark.mongodb.output.collection", parametrosEntrada.getProperty("MONGO_COLL") + "_BISECTINGKMEANS_" + (new SimpleDateFormat("yyyyMMddHHmmssSSS").format(new Date())));
				...
				JavaPairRDD<String, Vector> data = dataset.toJavaRDD().mapToPair(
						(in) -> {
							String par = in.getString(in.fieldIndex("VenueID")) +
										" " +
